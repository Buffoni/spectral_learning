...
class Autoencoder(Model):
  def __init__(self, latent_dim):
    ...
    self.encoder = tf.keras.Sequential([
      layers.Flatten(),
      layers.Dense(latent_dim, activation='tanh',kernel_regularizer=tf.keras.regularizers.l1(l1=0.00001)),
    ])
...
class SpecEncoder(Model):
  def __init__(self, latent_dim):
    ...
    self.encoder = tf.keras.Sequential([
      layers.Flatten(),
      Spectral(latent_dim, activation='tanh', is_base_trainable=True, is_diag_trainable=True, diag_regularizer='l1'),
    ])
    self.decoder = tf.keras.Sequential([
      layers.Dense(784, activation='softmax'),
      layers.Reshape((28, 28))
...
accuracy_perc_plot.get_figure().savefig("./test/mnist_REG_enc_elu.png")
plt.show()
